...
| P0.5 | **RBAC not enforced** | `middleware.ts` `EMPLOYER_ONLY_PATHS` = login-only, no role check; employer pages redirect client-side only (cosmetic). `profile/me` allows role→"admin" escalation; `applications/[id]` DELETE is IDOR + PATCH unscoped; signup lets client choose employer role; `opportunities/[id]` PATCH + admin/organizations POST accept raw bodies (mass assignment) | **FIXED (Phase 1.6, commit 5122e4f):** middleware server-side employer gate on `EMPLOYER_ONLY_PATHS` (role from `user_metadata.role` + `account_type:"provider"` fallback; non-employer → 403/redirect). Note: admin APIs intentionally NOT middleware-gated — admin console authenticates via `x-admin-password`/HMAC tokens with no Supabase session; `requireAdmin` stays the fail-closed gate. Escalation closed (`profile/me` rejects `"admin"`); `applications/[id]` DELETE owner-scoped (IDOR → 404) + PATCH whitelisted (`applicationStatusUpdateSchema`); signup `accountType` whitelisted; `opportunities/[id]` PATCH zod+mapped+resolver, `admin/organizations` POST strict live-columns `organizationCreateSchema` (replaced unused dead-column `adminOrganizationSchema`). New `validation-rbac.test.ts` (14 tests); 104 jest tests pass. Runtime role/IDOR behavior verified at code + unit level; owner can spot-check non-employer 403 on `/post-job` with a candidate session |
| P0.6 | **Security gaps** | Fail-open: `ai/expire` (guard inverted), `send-digest`. `revalidate` secret in query string. Individual scrapers unauthenticated + rate-limit-exempt with service-role writes. `employer/recommendations` ignores `is_profile_public`. `report-issue` no auth/rate-limit. Feed comment route reads `feed_posts.user_id` (v2 uses `author_id`). `platform_analytics` + `ai_usage_log` queried but don't exist live → broken analytics routes, silent AI log failures | **CLOSED (Phase 1.2 + 1.7):** fail-open guards inverted in Phase 1.2. Phase 1.7: all `/api/scrapers/*` behind `requireCronOrAdmin` (single guard in shared `runScraperRoute`; verified live 403); dead no-op `/api/revalidate` deleted (secret had leaked in query string via its self-fetch); feed comment notification lookup `user_id`→`author_id` (`user_id` doesn't exist live). `report-issue` already zod-validated + Upstash rate-limited at middleware + CSRF-exempt as a public form. `employer/recommendations` `is_profile_public` ignored + analytics repoint done in Phase 1.2/1.5 (see rows). |
...
**STATUS (2026-08-17):** steps 1–6 and 8 implemented in code (commits `b2447ff` + Phase 1.4 `c5df37a` + Phase 1.5 `5122e4f` + Phase 1.6 `74e3d59` + Phase 1.7 `d0cb240`): cron → real engine, fail-closed guards, verification v1 (evidence ledger + `unverified` default + no auto-promotion), P0.6 `platform_analytics`/`ai_usage_log` analytics repoint, P0.4 legacy-column route fixes, **org resolution v1** (step 4 — resolver module, shared `resolveOrganizationId()` insert helper, backfill script applied live: `organization_id IS NULL` 3,115 → 196, 95.2% → 6.0%), **schema reconciliation** (step 5 — every P0.4 register route now queries the live schema), **RBAC server-side** (step 6 — middleware employer gate on `EMPLOYER_ONLY_PATHS`, admin routes left to secret-based `requireAdmin` by design; `profile/me` escalation closed; `applications/[id]` IDOR closed + PATCH whitelisted; signup accountType whitelist; `opportunities/[id]` PATCH + `admin/organizations` POST zod/mapped), and **security sweep** (step 8 — scrapers behind `requireCronOrAdmin` verified live 403, dead `/api/revalidate` deleted, feed comment `author_id`). **FK RESTRICT** verified live on `saved_opportunities.opportunity_id` (deleting an opportunity with bookmarks now returns constraint violation, bookmarks preserved). Playwright E2E: 5/5 flows pass (network, connect, messages, header nav). `vercel.json` reverted to `cd frontend && npm run build` (the `--workspaces` variant timed out due to `file:` protocol resolution). Live-verified embeds/orders/searches; build passes; 104 jest tests. **🚨 SECURITY INCIDENT (2026-08-17):** `change-fk.js` (root) and `.opencode/mcp-servers/change-fk.js` committed with live Supabase Management API token + service role key for `jbqjipwanfsxyqkfrrpx` in plaintext — files deleted; owner must rotate both tokens immediately. `opportunity_verifications` DDL is authored but **not yet applied live** — owner must run it (Dashboard SQL editor or `supabase link` + `db push`). Step 7 (RLS audit — needs SQL/psql access, no postgres URL to agent) and step 9 (db2/Neon2 topology reconcile — db2 not MCP-accessible) pending owner tooling.
...
6. **RBAC server-side.** Middleware: for `EMPLOYER_ONLY_PATHS` + `/admin/*`, check role from `app_metadata.role` (server-side, via `getUser()`); non-employer → 403/redirect. Reuse `backend/api/src/auth` helpers — no new duplicated guards. Fix `profile/me` role escalation, `applications/[id]` ownership, signup role selection, raw-body mass assignment (`opportunities/[id]`, admin/organizations). *Verify: non-employer 403 on `/post-job` API; IDOR attempt returns 404.* **IMPLEMENTED (Phase 1.6, commit 5122e4f).** Deviation from plan: `/admin/*` is NOT middleware-gated — the admin console has no Supabase session (secret-based `x-admin-password`/HMAC), so gating on metadata role would lock out the legitimate flow; `requireAdmin` is the fail-closed gate on every admin route instead. Employer gate covers `EMPLOYER_ONLY_PATHS` (pages + APIs). Escalation/IDOR/signup/mass-assignment all fixed; runtime role checks verified at unit level (14 new `validation-rbac.test.ts` tests); owner spot-check: candidate session → `/post-job` should 403, `DELETE /api/applications/<not-mine>` → 404.
...
8. **Security sweep.** Scrapers behind `requireCronOrAdmin` + rate limits (remove middleware exemption); `revalidate` secret out of query string; `report-issue` auth + rate limit; feed comment `user_id`→`author_id`; fix `platform_analytics`/`ai_usage_log` (create live tables or repoint). *Verify: each fix has a test.* **IMPLEMENTED (Phase 1.7).** All `/api/scrapers/*` → `requireCronOrAdmin` in shared `runScraperRoute` (verified live: unauth GET → 403). `/api/revalidate` deleted (no-op; secret leaked via query-string self-fetch). Feed comment notification `author_id` fix. `report-issue` already zod + middleware-rate-limited. Rate-limit exemption for scrapers retained intentionally — the routes are now auth-guarded, so only cron/admin can invoke them; an additional per-request rate limit would only throttle the legitimate cron run.
