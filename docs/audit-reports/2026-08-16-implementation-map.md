...
**STATUS (2026-08-16):** steps 1–6 implemented in code (commits `b2447ff` + Phase 1.4 `c5df37a` + Phase 1.5 `5122e4f` + Phase 1.6): cron → real engine, fail-closed guards, verification v1 (evidence ledger + `unverified` default + no auto-promotion), P0.6 `platform_analytics`/`ai_usage_log` analytics repoint, P0.4 legacy-column route fixes, **org resolution v1** (step 4 — resolver module, shared `resolveOrganizationId()` insert helper, backfill script applied live: `organization_id IS NULL` 3,115 → 196, 95.2% → 6.0%), **schema reconciliation** (step 5 — every P0.4 register route now queries the live schema: `track-click` → Neon `click_events`; `cron/scrape-india|global` + `employer/jobs` + `admin/opportunities` write `organization_id`/`salary_range`/`apply_url` via the shared helper and never fabricate `verified`; `search/opportunities` + `ai/search` org filter via `organization_id`; applications/recommendations/scrape-health embed `organizations(name)` with wire-contract-preserving mapping; `scraper_sources`→`scrape_sources` + `last_scraped_at`→`last_scrape_at` (live column; impl was writing a dead one); admin verification enum trimmed to live CHECK values; `db/index.ts` comments + `neon/schema.sql` (trending_cache, keyword_stats) corrected; digest orders by live `view_count`), and **RBAC server-side** (step 6 — middleware employer gate on `EMPLOYER_ONLY_PATHS`, admin routes left to secret-based `requireAdmin` by design; `profile/me` escalation closed; `applications/[id]` IDOR closed + PATCH whitelisted; signup accountType whitelist; `opportunities/[id]` PATCH + `admin/organizations` POST zod/mapped). Live-verified embeds/orders/searches; build passes; 104 jest tests. A consolidated current-state DDL migration for db1/db2/Neon1 is intentionally NOT authored as a single file — the code-vs-schema drift (the register's root cause) is fixed at the code layer and the live schema is the documented truth in `db/index.ts` + `neon/schema.sql`; a mega-migration would be unverifiable without postgres access. `opportunity_verifications` DDL is authored but **not yet applied live** — owner must run it (Dashboard SQL editor or `supabase link` + `db push`). Steps 7–9 pending.
