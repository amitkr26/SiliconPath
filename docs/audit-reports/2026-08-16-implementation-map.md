...
**STATUS (2026-08-16):** steps 1–4 implemented in code (commits `b2447ff` + Phase 1.4): cron → real engine, fail-closed guards, verification v1 (evidence ledger + `unverified` default + no auto-promotion), P0.6 `platform_analytics`/`ai_usage_log` analytics repoint, P0.4 legacy-column route fixes touched by them, and **org resolution v1** (step 4): resolver module `frontend/src/lib/organizations/resolve.ts` (domain/ATS-token/name/title evidence + person-name guard; now the single source of truth — `utils.ts` `inferAuthenticOrganization` delegates to it, scrape insert path resolves via it, ATS orphan `ats-adapter.ts` deleted), idempotent backfill script `frontend/scripts/backfill-organization-ids.ts` (dry-run default, `--apply` to write), jest suite `organizations-resolve.test.ts` (11 tests). **Backfill applied live: `organization_id IS NULL` 3,115 → 196 (95.2% → 6.0%)**; all 2,919 assignments reference existing org rows; residual 196 = genuine org-table gaps (e.g. IIT Hyderabad, BEL, Cirrus — no org row exists) + no-URL evidence, left unresolved by design. `opportunity_verifications` DDL is authored but **not yet applied live** — no Supabase postgres URL available to the agent; owner must run the migration (Dashboard SQL editor or `supabase link` + `db push`). Steps 5–9 pending.
