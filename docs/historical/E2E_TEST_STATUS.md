...
| 10 | **Employer Job Lifecycle** | Employer creates job on `/employer/post-job`, pauses, resumes, deletes | Job created (201), persisted in DB, public stream synced, pause/resume updates `is_active` | ✅ PASS (100%) |
| 11 | **Employer ATS Applicant Pipeline** | Candidate applies to employer job -> Employer reviews in ATS (`/employer/applicants`) -> moves stage -> writes notes | Visible in ATS pipeline, stage advances (`applied` -> `screening` -> `shortlisted` -> `interview` -> `accepted`), notes persisted in DB | ✅ PASS (100%) |
| 12 | **Candidate Talent Sourcing & Reachout** | Employer searches `/employer/talent` -> opens `/employer/talent/[username]` -> sends direct invitation | Verified `user_profiles` queried, direct invitation creates `conversations` + `messages` + `notifications` | ✅ PASS (100%) |
| 13 | **Employer Workspace & Settings** | Employer updates `/employer/company`, manages seats in `/employer/team`, updates `/employer/settings` | Company details, seats, and preferences persisted in DB | ✅ PASS (100%) |
| 14 | **Employer Analytics Engine** | Employer navigates to `/employer/analytics` | SQL-aggregated recruitment funnel (Applied, Screening, Shortlisted, Interview, Hired) dynamically calculated | ✅ PASS (100%) |