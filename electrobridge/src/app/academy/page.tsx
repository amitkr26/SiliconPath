...
import {
  ...
  Lock, ChevronRight, Zap, Play, Check, AlertCircle, RefreshCw
} from "lucide-react";
...
// Guard: reject a promise if it takes longer than `ms` so the UI never hangs.
...
export default function AcademyDashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // Tracks are the critical path. Guard with a timeout.
      const tracksData = await withTimeout(getTracks(), 10000, "getTracks");
      setTracks(tracksData);

      // Progress is best-effort; failures here should not blank the page.
      ...
      setLoading(false); // ALWAYS clear loading — no more infinite spinner
    }
  }
  ...
  // ---- Error state (never blank / never infinite) ----
  if (error) {
    return (
                      ...
                      <div className="space-y-2">
                        ...
                        {allDaysCompleted && !isTrackPassed ? (
                          <Link
                            ...
                            className="mt-2 w-full py-2 rounded-lg text-center text-sm font-bold bg-amber-400 text-gray-900 hover:bg-amber-300 transition-colors flex items-center justify-center gap-1.5"
                          >
                          ...
                          <Link
                            ...
                            className="mt-2 w-full py-2 rounded-lg text-center text-sm font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors flex items-center justify-center gap-1.5"
                          >
