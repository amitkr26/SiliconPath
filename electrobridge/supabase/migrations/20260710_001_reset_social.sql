...
-- NOTE: `current_role` is a reserved word; the title column is `job_title`.
-- ============================================================================
...
CREATE POLICY "read companies" ON company_profiles FOR SELECT USING (true);
CREATE POLICY "manage own company" ON company_profiles FOR ALL USING (auth.uid() = owner_id);
CREATE POLICY "see own connections" ON connections FOR SELECT USING (auth.uid() = requester_id OR auth.uid() = addressee_id);
CREATE POLICY "create connection" ON connections FOR INSERT WITH CHECK (auth.uid() = requester_id);
CREATE POLICY "update own connection" ON connections FOR UPDATE USING (auth.uid() = requester_id OR auth.uid() = addressee_id);
CREATE POLICY "read feed" ON feed_posts FOR SELECT USING (true);
CREATE POLICY "manage own posts" ON feed_posts FOR ALL USING (auth.uid() = author_id);
CREATE POLICY "manage own reactions" ON post_reactions FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "see own conversations" ON conversations FOR SELECT USING (auth.uid() = participant_a OR auth.uid() = participant_b);
CREATE POLICY "see own messages" ON messages FOR SELECT USING (EXISTS (SELECT 1 FROM conversations c WHERE c.id = conversation_id AND (c.participant_a = auth.uid() OR c.participant_b = auth.uid())));
CREATE POLICY "send messages" ON messages FOR INSERT WITH CHECK (auth.uid() = sender_id);
CREATE POLICY "manage saved" ON saved_opportunities FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "manage applications" ON job_applications FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "manage progress" ON academy_user_progress FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "manage results" ON academy_assessment_results FOR ALL USING (auth.uid() = user_id);
