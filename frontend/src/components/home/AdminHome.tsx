  ...
  return (
    <div className="min-h-screen bg-bg-primary pb-20">
      ...
      <section className="bg-white border-b border-slate-200 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 border border-red-200 text-red-700 text-xs font-semibold uppercase tracking-wider mb-3 shadow-xs">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-8 pt-6 border-t border-slate-200">
            <Link href="/admin?tab=opportunities" className="p-4 bg-white border border-slate-200 rounded-xl shadow-card hover:border-blue-300 hover:shadow-md transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600">Active Verified</span>
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
              </div>
              <p className="text-2xl font-bold text-slate-900 mt-1">{stats.activeOpportunities}</p>
            </Link>

            <Link href="/admin?tab=moderation" className="p-4 bg-white border border-slate-200 rounded-xl shadow-card hover:border-blue-300 hover:shadow-md transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600">Pending Moderation</span>
                <AlertTriangle className="w-4 h-4 text-amber-600" />
              </div>
              <p className="text-2xl font-bold text-slate-900 mt-1">{stats.pendingVerification}</p>
            </Link>

            <Link href="/admin?tab=opportunities" className="p-4 bg-white border border-slate-200 rounded-xl shadow-card hover:border-blue-300 hover:shadow-md transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600">Expired / Archived</span>
                <Clock className="w-4 h-4 text-slate-400" />
              </div>
              <p className="text-2xl font-bold text-slate-900 mt-1">{stats.expiredOpportunities}</p>
            </Link>

            <Link href="/admin?tab=scrapers" className="p-4 bg-white border border-slate-200 rounded-xl shadow-card hover:border-blue-300 hover:shadow-md transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600">Scraper Fleet</span>
                <Radio className="w-4 h-4 text-purple-600" />
              </div>
              <p className="text-2xl font-bold text-slate-900 mt-1">{stats.activeScrapers} Active</p>
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <h2 className="text-lg font-bold text-slate-900 mb-4">Administration Workflows</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link
            href="/admin?tab=moderation"
            className="p-6 bg-white border border-slate-200 rounded-2xl shadow-card hover:border-blue-300 hover:shadow-elevated transition-all"
          >
