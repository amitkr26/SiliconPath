...
const DEEPSEEK_COST_RATES: Record<string, { input: number; output: number }> = {
...
export class DeepSeekProvider extends BaseProvider {
  ...
  async chat(
    ...
    try {
      ...
      const data = await this.fetchJson<DeepSeekChatResponse>("/chat/completions", body);
  ...
  async *stream(
    ...
    try {
      for await (const raw of this.parseSseStream(res)) {
        ...
        try {
          chunk = JSON.parse(raw) as DeepSeekStreamChunk;
        } catch {
          continue;
        }
  ...
  async embed(
    ...
    throw new Error(`${this.name}: Embeddings are not supported by DeepSeek.`);
  }
  ...
  protected getCostRates(model: string): { input: number; output: number } | null {
    return DEEPSEEK_COST_RATES[model] ?? null;
  }
