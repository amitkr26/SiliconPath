...
const TOGETHER_COST_RATES: Record<string, { input: number; output: number }> = {
  "meta-llama/Llama-3.3-70B-Instruct-Turbo": { input: 0.88, output: 0.88 },
  "meta-llama/Llama-3.3-70B-Instruct-Turbo-for-Google": { input: 0.88, output: 0.88 },
  ...
  "mistralai/Mistral-7B-Instruct-v0.3": { input: 0.1, output: 0.1 },
  "Qwen/Qwen2.5-72B-Instruct-Turbo": { input: 1.2, output: 1.2 },
  "google/gemma-2-27b-it": { input: 0.35, output: 0.35 },
  "deepseek-ai/DeepSeek-R1": { input: 0.55, output: 2.19 },
  "deepseek-ai/DeepSeek-V3": { input: 0.27, output: 1.1 },
  "databricks/dbrx-instruct-preview": { input: 0.88, output: 0.88 },
};
...
export class TogetherProvider extends BaseProvider {
  ...
  async chat(
    ...
    try {
      ...
      const data = await this.fetchJson<TogetherChatResponse>("/chat/completions", body);
  ...
  async *stream(
    ...
    try {
      for await (const raw of this.parseSseStream(res)) {
        ...
        try {
          chunk = JSON.parse(raw) as TogetherStreamChunk;
        } catch {
          continue;
        }
  ...
  async embed(
    ...
    throw new Error(`${this.name}: Embeddings are not supported by Together AI via chat completions.`);
  }
  ...
  protected getCostRates(model: string): { input: number; output: number } | null {
    return TOGETHER_COST_RATES[model] ?? null;
  }
