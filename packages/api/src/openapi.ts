...
export function zodToSchema(zodSchema: z.ZodTypeAny): SchemaObject {
  // Simplified for zod v4 - use basic type checking via constructor name
  // Cast to any to access internal properties not exposed in types
  ...
  switch (typeName) {
    ...
    case "ZodNullable": {
      const unwrappedSchema = zodToSchema(schema.unwrap());
      return { ...unwrappedSchema, nullable: true };
    }
    case "ZodDefault": {
      const unwrappedSchema = zodToSchema(schema.unwrap());
      const def = (schema as any)._def;
      const defaultVal = def?.defaultValue;
      return { ...unwrappedSchema, default: defaultVal };
    }
    case "ZodEnum":
