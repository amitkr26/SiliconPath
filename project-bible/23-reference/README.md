...
This directory contains comprehensive reference documents for the SiliconPath platform. Use these files as the single source of truth for environment variables, API keys, URL schemes, commands, and patterns.
...
- **Monorepo**: Single repo at `github.com/getsiliconpath/siliconpath`
- **Frontend URL**: `https://siliconpath.vercel.app` (production)
- **Backend URL**: Render-generated URL (check Render dashboard)
- **Primary Database**: Supabase DB1 — `siliconpath` project
- **Secondary Database**: Supabase DB2 — `siliconpath-social` project
- **Analytics Database**: Neon — `siliconpath-analytics` project
- **Email Provider**: Resend — project `siliconpath`
...
# Docker
docker build -t siliconpath-backend ./backend
docker run -p 4000:4000 siliconpath-backend
```
