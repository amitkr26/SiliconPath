...
| `backend/worker` | Independently runnable process workloads (`node --import tsx dist/index.js news`) — Phase 6: news RSS sync | — | IMPLEMENTED (17 node:test), runs the shared `content/news-sync` module; force-exits after summary (2026-08-20 hang fix); production-mode run evidence 2026-08-20 (KNOWN_ISSUES #12). NOT scheduled on Render (Phase 7 — no Render cron; Vercel cron stays the production owner). |
...
**[TRANSITION — open parity gaps]** (tracked in `backend/docs/BACKEND-PARITY-MATRIX.md`, supersedes `API-PARITY.md`): scraper fleet not ported beyond news RSS (Phase 7: inventory done in `project-bible/09-scrapers/REPLICA-MIGRATION-MATRIX.md` — opportunity scrapers/ATS stay in the frontend by owner mandate), PATCH `/api/v1/profiles/me` deferred, DB2 (`supabase2Admin`) client wired but unused, admin breadth beyond `/stats`, academy/misc.
