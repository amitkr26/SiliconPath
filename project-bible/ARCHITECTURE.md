...
**[CURRENT — Phase 8 (2026-08-20)]** Vercel remains the production platform. Render is an **independently deployed backend replica** (`backend/server` + `backend/api` + `backend/ai-gateway` + independently runnable `backend/worker`). The production frontend does NOT depend on Render (no Vercel→Render calls in any production code path); production Next.js API routes and the production Vercel cron remain active and own all production traffic. Supabase remains the shared source of truth. Render is **free-tier only** — no Render Cron Job, no Background Worker, no paid plan (`render.yaml`, Phase 7). Authoritative parity evidence: `backend/docs/BACKEND-PARITY-MATRIX.md`. **Phase 8:** the worker gained a second, independent command — the ISRO government scraper replica (`backend/worker/dist/index.js isro`) — with the production normalization/dedup/verification contract ported verbatim; the scraper-fleet replication pattern is now proven end-to-end (parse → dedup → insert with `pending` status → health persistence → idempotent re-runs).
...
- **[CURRENT — Phase 7]** The Express server replicates the AI endpoint breadth: `POST /api/v1/ai/{insights,chat,match,search,summarize}` with the same grounding guards (record-listing, no-match fallback, URL allowlist), telemetry via `setLogger` → `ai_usage_log` on db1 (Phase 6.6 verified live: summarize 200, groq `qwen/qwen3.6-27b`, telemetry row present). Frontend-only AI surface (classify, enhance, expire, opportunity-summary) stays on Vercel.
...
| `backend/ai-gateway` | 9-provider AI fallback chain | — (library) | IMPLEMENTED, jest tests (15); groq model `qwen/qwen3.6-27b` (2026-08-20, was retired `llama-3.1-8b-instant`) |
| `backend/server` | Express 4 REST API — the only runnable component | 8080 (`PORT` env) | IMPLEMENTED (46 node:test), DEPLOYED 2026-08-20 (live, auto-deploy), graceful SIGTERM shutdown |
| `backend/worker` | Independently runnable process workloads (`node --import tsx dist/index.js news|isro`) — Phase 6: news RSS sync; Phase 8: ISRO government scraper replica | — | IMPLEMENTED (30 node:test — 17 news + 13 ISRO), runs the shared `content/news-sync` module + the ISRO scraper (`scrapers/isro.ts`, pure parser + deps-injected runner); force-exits after summary (2026-08-20 hang fix); production-mode run evidence 2026-08-20 (news: KNOWN_ISSUES #12; isro: 2 live runs, 18 fetched / 0 inserted — honest parity with the frontend's 0-row live output, KNOWN_ISSUES #17/#18). NOT scheduled on Render (Phase 7 — no Render cron; Vercel cron stays the production owner). |

**`backend/server` route inventory (CURRENT):** `GET /health` (liveness) + `GET /health/ready` (readiness probe, 503 `DB_UNAVAILABLE` without a DB); `GET /api/v1/opportunities` (+ `/:idOrSlug`); `GET /api/v1/profiles/me` (Bearer) + `/:username`; `GET /api/v1/organizations` (+ `/:slug`); `GET /api/v1/news` (+ `/:slug`); `GET/POST /api/v1/applications` + `PATCH/DELETE /:id`; `GET/POST /api/v1/saved-opportunities` + `DELETE /:id`; `POST /api/v1/ai/{chat,match,search,summarize,insights}` (Bearer + ai rate limit; gateway exhaustion → 502 `AI_UNAVAILABLE`); `GET /api/v1/admin/stats` (timing-safe `X-Admin-Password`, admin rate limit 20/min); `POST /api/v1/auth/signup` + `GET /check-username`; `GET /api/v1/search` + `/people`; `GET /api/v1/feed` + `POST/GET/PATCH/DELETE /posts[/:id]` + `like/comment/repost`; `GET /api/v1/network/{connections,status/:userId,connect,suggestions,follow/:userId,followers,following}`; `GET /api/v1/notifications` + mark paths; `GET /api/v1/messages` + `/:conversationId` + `with/:userId`; `GET /api/v1/cron/news-sync` (timing-safe Bearer `CRON_SECRET`; upserts `news_articles` onConflict `url` via the shared module). Envelope `{success,data,pagination}` / `{success:false,error:{code,message,details?}}`. Express rate limiting via the shared api-lib presets (api 120/min, auth 10/min, search 30/min, ai 20/min, admin 20/min) through a Web-Request header shim. Dockerfile: node:22-alpine (supabase-js ≥2.110 needs native WebSocket), non-root `USER node`, HEALTHCHECK, EXPOSE 8080, `.dockerignore` excludes the host node_modules. `dist/` is generated + gitignored. Deployment decision: Render Docker web service (`render.yaml`, Phase 7) — free tier, no Render cron. **DEPLOYED 2026-08-20** (Phase 6.5): web service live at `https://berojgardegreewala-backend.onrender.com` (auto-deploy on push to main; `plan: free` (render.yaml Phase 7); the earlier `starter` attempt was rejected 402 and is moot — no paid Render infrastructure is required (KNOWN_ISSUES #14 CLOSED). `/health` + `/health/ready` verified; `/api/v1/cron/news-sync` verified in production (58 articles inserted, re-runs insert 0). AI fixed 2026-08-20 (Phase 6.6): groq model -> qwen/qwen3.6-27b, backend + frontend 200, telemetry verified (no credentials in logs). Worker entrypoint: force-exit hang fix + production-mode runs (Run A +5 rows -> 285; Run B exit 0, 0 dupes). Opportunity embed maps slug/website. Render cron still BLOCKED (billing #14; re-verified 402 + empty billingCheckState). Vercel cron remains the production owner.

**[TRANSITION — open parity gaps]** (tracked in `backend/docs/BACKEND-PARITY-MATRIX.md`, supersedes `API-PARITY.md`): scraper fleet ported beyond news RSS = news + ISRO only (Phase 8: government scraper pattern proven; DRDO/CSIR are the next live-verified candidates — inventory in `project-bible/09-scrapers/REPLICA-MIGRATION-MATRIX.md`; remaining opportunity/ATS scrapers stay in the frontend by owner mandate), PATCH `/api/v1/profiles/me` deferred, DB2 (`supabase2Admin`) client wired but unused, admin breadth beyond `/stats`, academy/misc.
