...
`backend/api` (workspace) is the framework-less TypeScript library consumed by BOTH the Next.js app and the Express server: `response/`, `error/` (AppError hierarchy), `auth/` (getUser/requireAuth/requireAdmin/requireCron), `validation/` (zod + Supabase query builders), `rate-limit/` (in-memory buckets), `cache/`, `openapi/` (spec of the frontend `/api` surface — `backend/api/openapi.json`, 51 paths), `content/` (taxonomy/dates/status/sources/dedup/seo/quality — used by scrapers/ingest). Note: the Express server uses its own `{success,error}` envelope middleware and AppError classes from this package; the Next routes use the package's Response helpers. Both are documented in `backend/docs/API-PARITY.md`.
...
|---|---|---|---|
| `backend/api` | Framework-less shared library (response/error/auth/validation/rate-limit/cache/openapi/content) | — (library) | IMPLEMENTED, jest tests (97) |
| `backend/ai-gateway` | 9-provider AI fallback chain | — (library) | IMPLEMENTED, no tests yet |
| `backend/server` | Express 4 REST API — the only runnable component | 8080 (`PORT` env) | IMPLEMENTED (16 node:test), parity IN PROGRESS |

**`backend/server` route inventory (CURRENT):** `GET /health`; `GET /api/v1/opportunities` (+ `/:idOrSlug`); `GET /api/v1/profiles/me` (Bearer) + `/:username`; `GET /api/v1/organizations` (+ `/:slug`); `GET /api/v1/news` (list only); `GET/POST /api/v1/applications` + `PATCH/DELETE /:id`; `GET/POST /api/v1/saved-opportunities` + `DELETE /:id`; `POST /api/v1/ai/insights`; `GET /api/v1/admin/stats` (x-admin-password). CORS allow-list via `ALLOWED_ORIGINS`. Envelope `{success,data,pagination}` / `{success:false,error:{code,message,details?}}`. Dockerfile: node:20-alpine, EXPOSE 8080. `dist/` is generated + gitignored. No rate limiting yet, no scrapers, no cron.

**[TRANSITION — open parity gaps]** (tracked in `backend/docs/API-PARITY.md`): social layer (feed/network/messages/notifications — `supabase2Admin` DB2 client wired but unused), AI endpoint breadth + usage logging, cron/scraper port (news RSS sync first), search + news `:slug` + `auth/signup`, admin breadth, academy/misc.
