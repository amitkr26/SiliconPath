...
- **2026-08-18 — Production Social Networking Bug Fix (follow/connect/messages/feed).** All four reported production failures fixed at root cause and verified against live DB1 (`aqauempuwmbizqoaolop`):
  ...
  - Verified: `npm run build` passes, 104 jest tests pass. Deploy via Vercel git integration (push-triggered; CLI `vercel deploy --prod` races the git deployment and must not be used). Live verification of follow/connect/message/feed on production in progress — see `project-bible/E2E_TEST_STATUS.md`.
