...
- **Description**: Extract `electrobridge/src/lib/ai/` into `packages/ai-gateway/`. Create unified `AIProvider` interface: `call(prompt, options): Promise<AIResponse>`, `isAvailable(): boolean`, `getQuota(): ProviderQuota`. Implement for all 7 providers. Export as `@siliconpath/ai-gateway`
- **Dependencies**: 1.1.1.1 (monorepo workspace)
- **Acceptance Criteria**: Both apps can import `@siliconpath/ai-gateway`. All 7 providers implement the interface. Fallback chain works. Usage is logged
...
- **Description**: Update `apps/web/src/lib/ai/` to import from `@siliconpath/ai-gateway`. Remove local provider implementations. Update all 8 AI feature files to use gateway
- **Dependencies**: 6.1.1.1
...
- **Description**: Update `apps/api/src/lib/` to import from `@siliconpath/ai-gateway`. Remove `ai-providers.ts`. Backend scrapers use gateway for AI parsing
- **Dependencies**: 6.1.1.1
...
- **Description**: Delete `electrobridge/src/lib/ai/providers.ts` (after confirming migration). Delete `backend/src/lib/ai-providers.ts`
