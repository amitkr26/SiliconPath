...
- **Description**: Extract `backend/src/scrapers/source-config.ts` into `packages/config/`. Move frontend's `companies.json` and `institutions.json` data into the same package as TypeScript. Export as `@siliconpath/config`. Ensure all 500+ sources are represented
...
- **Description**: Move India-specific scraper files from `electrobridge/src/lib/scrapers/` (drdo-scraper.ts, isro-scraper.ts, csir-scraper.ts, govt-scraper.ts, india-psu-scraper.ts, india-academic-scraper.ts, international-academic-scraper.ts, global-semiconductor-scraper.ts, fellowship-scraper.ts, deep-scraper.ts) to `apps/api/src/scrapers/adapters/`. Refactor to match backend adapter interface
