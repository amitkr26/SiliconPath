...
| File | Reason | Action |
|------|--------|--------|
| `electrobridge/skills-lock.json` | AI agent skill lock file, not referenced in build | Delete or archive |
| `electrobridge/src/config/scrapers/companies.json` | Duplicated by `backend/src/scrapers/source-config.ts` (backend is primary scraper) | Archive or delete |
| `electrobridge/src/config/scrapers/institutions.json` | Same duplication issue | Archive or delete |
| `electrobridge/public/auth-sync.html` | Static HTML page, purpose unclear | Verify and delete if unused |
| `electrobridge/public/next.svg` | Default Next.js logo, not used in branding | Delete |
| `electrobridge/public/vercel.svg` | Default Vercel logo, not used in branding | Delete |
...
| Aspect | Frontend (`electrobridge/src/lib/scrapers/`) | Backend (`backend/src/scrapers/`) |
|--------|----------------------------------------------|-----------------------------------|
...
Both `electrobridge/src/lib/ai/providers.ts` and `backend/src/lib/ai-providers.ts` implement the same 7-provider fallback chain with nearly identical logic. This violates the AI Gateway principle and creates maintenance burden.

### 2.3 Duplicate Source Configurations

| Config Source | Format | Count | Overlap |
|--------------|--------|-------|---------|
| `electrobridge/src/config/scrapers/companies.json` | JSON | 70+ | High overlap with backend |
| `electrobridge/src/config/scrapers/institutions.json` | JSON | 30+ | High overlap with backend |
| `backend/src/scrapers/source-config.ts` | TypeScript | 500+ | Master source |

### 2.4 Duplicate Wipe Scripts

`electrobridge/scripts/wipe_data.js` exists alongside `electrobridge/scripts/wipe_data_cjs.js` (which was deleted in cleanup). One should be sufficient.

### 2.5 Duplicate Lock Files

`electrobridge/pnpm-lock.yaml` was present alongside `package-lock.json` (pnpm-lock.yaml was deleted in cleanup). Dual lock files indicate package manager switching.
