...
├── apps/
│   ├── web/                   ← RENAMED from electrobridge/
│   │   ├── src/
...
|--------|-------------|--------|
| `electrobridge/src/lib/ai/providers.ts` | `packages/ai-gateway/src/providers.ts` | Shared AI Gateway |
| `electrobridge/src/lib/ai/safe-parse.ts` | `packages/ai-gateway/src/safe-parse.ts` | Shared utility |
| `electrobridge/src/lib/scrapers/` (India scrapers) | `apps/api/src/scrapers/adapters/` | Consolidate all scrapers in backend |
| `electrobridge/src/config/scrapers/` | `packages/config/` | Single source of truth |
| `backend/src/lib/ai-providers.ts` | `apps/api/src/lib/ai/providers.ts` (or import from package) | Use shared AI Gateway |
| `electrobridge/supabase/migrations/` | `packages/database/migrations/` | Centralized migrations |
| `electrobridge/supabase/seed/` | `packages/database/seed/` | Centralized seeds |
...
| `electrobridge/src/config/scrapers/companies.json` | Superseded by config package |
| `electrobridge/src/config/scrapers/institutions.json` | Superseded by config package |
| `electrobridge/public/next.svg` | Default Next.js logo, unused |
| `electrobridge/public/vercel.svg` | Default Vercel logo, unused |
| `electrobridge/public/auth-sync.html` | Purpose unclear, likely legacy |
| `electrobridge/skills-lock.json` | AI agent config, not needed in repo |
| `electrobridge/supabase/migrations/20260710_000_reset_core.sql` | Destructive reset migration, dangerous |
| `electrobridge/supabase/migrations/20260710_001_reset_social.sql` | Destructive reset migration, dangerous |
| `electrobridge/scripts/wipe_data.js` (or migrate to `scripts/db/`) | Destructive, should not be accidentally run |

## 5. Files to Merge

| Files | Merge Into | Reason |
|-------|-----------|--------|
| `backend/src/scrapers/source-config.ts` + `electrobridge/src/config/scrapers/*.json` | `packages/config/source-config.ts` | Single source of truth for 500+ sources |
| `electrobridge/src/lib/ai/providers.ts` + `backend/src/lib/ai-providers.ts` | `packages/ai-gateway/src/` | Single AI Gateway |
...
|------|-----------|--------|
| `electrobridge/src/lib/utils.ts` | Multiple domain-specific utils | Monolithic utils file with unrelated functions |
| `electrobridge/src/types/index.ts` | `packages/types/*.ts` | All 28+ interfaces in one file |
| `electrobridge/src/lib/ai/providers.ts` | Gateway + providers + types | Separate concerns |
| `backend/src/scrapers/source-config.ts` | `packages/config/*` | 500+ sources in one file is unwieldy |
| `electrobridge/src/app/api/` | By domain module | Currently flat, should be domain-organized |
...
| `electrobridge/src/lib/scrapers/opportunity-scraper-impl.ts` | Should be migrated to backend |
...
| P0-P2 | 2 | Extract `packages/types/` from `electrobridge/src/types/index.ts` | 1 day |
...
| P3-P1 | 11 | Rename `electrobridge/` to `apps/web/` | 1 day |
